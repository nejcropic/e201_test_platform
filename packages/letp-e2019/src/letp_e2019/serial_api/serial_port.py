...
def convert_hex_string_to_byte_array(hex_string: str) -> bytearray:
    if len(hex_string) % 2 != 0:
        raise ValueError(f"The binary key cannot have an odd number of digits: {hex_string}")

    data = bytearray(len(hex_string) // 2)
    # pylint: disable=consider-using-enumerate
    for index in range(len(data)):
        byte_value = hex_string[index * 2 : index * 2 + 2]
        data[index] = int(byte_value, 16)
    return data
